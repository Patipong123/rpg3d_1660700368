...
public class UIManager : MonoBehaviour
{
    ...
    //------------------------------- JoinParty -----------------
    [SerializeField]
    private Hero curHeroToJoint = null;
    ...
    public void ShowInventory()
    {
        ...
        for (int i = 0; i < hero.InventoryItem.Length; i++)
        {
            if (hero.InventoryItem[i] != null)
            {
                ...
                itemDrag.Item = hero.InventoryItem[i];
                itemDrag.IconParent = slots[i].transform;
                itemDrag.Image.sprite = hero.InventoryItem[i].Icon;
            }
        }
    }
    ...
    {
        index++;
        dialogueText.text = QuestManager.instance.NextDialouge(index);

        if (QuestManager.instance.CheckLastDialouge(index))
        {
            btnNext.SetActive(false);
    ...
    private void SetupPartyItem(Hero hero) 
    {
        curShopHero = hero;
        ...
        for (int i = 0; i < 16; i++) 
        {
            if (hero.InventoryItem[i] != null) 
            {
                ...
                itemInShop.ID = i;
                itemInShop.Item = hero.InventoryItem[i];
    ...
    public void PreparShopPanel(Npc npc, Hero hero) 
    {
        ClearShopPanel();
        SetupShopItem(npc);
        SetupPartyItem(hero);
    ...
    public void SellItemToShop() 
    {
        ...
        if (curShopNpc.NpcMoney >= totalPrice) 
        {
            foreach (GameObject obj in toSellCardList) 
            {
                ...
                curShopHero.InventoryItem[itemInShop.ID] = null;
    ...
    {
        PartyManager.instance.HeroJoinParty(curHeroToJoint);
        MapToggleAvatar();
        curHeroToJoint = null;
        ToggleDialogueBox(false);
    }
